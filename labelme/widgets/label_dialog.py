...
class LabelDialog(QtWidgets.QDialog):
    ...
    def popUp(self, text=None, move=True, flags=None, group_id=None, description=None):
        ...
        self.edit.setFocus(QtCore.Qt.OtherFocusReason)  # 修复 popUp 后回车后,名称改变的问题
        if move:
            # 防止 popUp 窗口越界
            screen_width = QtWidgets.QDesktopWidget().screenGeometry().width()
            screen_height = QtWidgets.QDesktopWidget().screenGeometry().height()

            current_cursor_x = QtGui.QCursor.pos().x()
            current_cursor_y = QtGui.QCursor.pos().y()
            if current_cursor_x + self.width() > screen_width:
                current_cursor_x = screen_width - self.width()
            if current_cursor_y + self.height() > screen_height:
                current_cursor_y = screen_height - self.height() - self.height()

            self.move(current_cursor_x, current_cursor_y)
        if self.exec_():
            return (
                self.edit.text(),
                self.getFlags(),
