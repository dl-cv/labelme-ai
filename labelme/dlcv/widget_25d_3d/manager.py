...
class Proj3DManager(ProjManagerBase):
    ...
    def is_3d_data(self, img_path: str) -> bool:
        ...
        return (gray_img_path.endswith(f'_G{suffix}')
                and depth_img_path.endswith(f'_H{suffix}')
                and suffix in self.suffixes_3D)
...
class Proj2_5DManager(ProjManagerBase):
    ...
    def get_img_name_list(self, img_path: str) -> list[str]:
        ...
        return [
            ...
            for img_path, json_file in self._file_to_json.items()
...
class ProjManager:
    ...
    def current_manager(self) -> ProjManagerBase:
        ...
        if STORE.main_window:
            proj_type = STORE.main_window.parameter.child(
                "proj_setting", "proj_type").value()

        # 返回对应的管理器，默认为 NORMAL
        return self._strategies.get(proj_type,
                                    self._strategies[ProjEnum.NORMAL])
